...
#pragma omp parallel for
	...
	LowResfile.close();
	//string traindata = "TrainData.bin";
	//WriteDataBinaryFile(TrainFeatureData, traindata);
#else
	string traindata = "TrainData.bin";
	ReadDataBinaryFile(TrainFeatureData, traindata);
#endif

	
}
void SuperOperation::WriteDataBinaryFile(cv::Mat& Data, string &fname)
{
	//std::ofstream myFile1("TrainFeatureData.bin", std::ios::out | std::ios::binary);
	std::ofstream myFile1(fname.c_str(), std::ios::out | std::ios::binary);
	for (int r = 0; r < Data.rows; r++)
	{
		myFile1.write(reinterpret_cast<char*>(Data.ptr(r)), Data.cols*Data.elemSize());
	}
	myFile1.close();
}
void SuperOperation::ReadDataBinaryFile(cv::Mat& Data, string &fname)
{
	float *buf = new float[Data.rows*Data.cols];
...
void SuperOperation::ReadFilename()
{
	...
	cout << TESTFILENAME.size() << endl;
	cout << TRAINFILENAME.size() << endl;
}
...
void SuperOperation::PatchtableBuild()
{
	...
	//p->limit = 500e6;
	//p->flann_checks =256;
	//p->flann_eps = 0.5;//1.5 will crash.
	//p->calc_exact_dist = true;
    p->kcoherence = 40;
	p->prop_iters = 4;
	//p->spatial = 80;
	//p->coherence_spatial = 0;
	p->grid_ndims = 10;
	p->ndims = im_PCADIMS;
	p->nchroma = 0;
...
void SuperOperation::PatchTableMethod()
{
		...
		/*string traindata = "TestData.bin";
		WriteDataBinaryFile(TestFeatureData, traindata);
		string trainOriginaldata = "TestOriginalData.bin";
		WriteDataBinaryFile(TestFeatureDataOriginal, trainOriginaldata);*/
#else
		string traindata = "TestData.bin";
		ReadDataBinaryFile(TestFeatureData, traindata);
		string trainOriginaldata = "TestOriginalData.bin";
		WriteDataBinaryFile(TestFeatureDataOriginal, trainOriginaldata);
#endif
		ofstream LowResfile("time.txt", std::ofstream::out | std::ofstream::app);
		...
		for (int r = 0; r < (lowRows - TrainImgLRSize + 1); r++)
		{
			for (int c = 0; c < (lowCols - TrainImgLRSize + 1); c++)
			{
				...
				for (int i = 0; i < 1; i++)
				{
					...
					{
						cout << "error: trainLum's size is more than one" << endl;
					}*/
					...
					resize(testemphr, testemp, cv::Size(TrainImgLRSize, TrainImgLRSize), 0, 0, cv::INTER_CUBIC);
					for (int r = 0; r < TrainImgLRSize; r++)
					{
						for (int c = 0; c < TrainImgLRSize; c++)
						{
		...
		cv::resize(im_TestCR, CR, cv::Size(im_TestCB.cols * im_DownSampleFactor, im_TestCB.rows * im_DownSampleFactor), 0, 0, cv::INTER_CUBIC);
		cv::bilateralFilter(CR, CRt, -1.0, 0.1, im_DownSampleFactor + 2);
		cv::resize(im_TestCB, CB, cv::Size(im_TestCB.cols * im_DownSampleFactor, im_TestCB.rows * im_DownSampleFactor), 0, 0, cv::INTER_CUBIC);
	...
	float MaxValue = -10000.0, MinValue = 100000.0;
		...
		for (int r = 0; r < lowRows - TrainImgLRSize + 1; r++)
		{
			for (int c = 0; c < lowCols - TrainImgLRSize + 1; c++)
			{
				...
				clock_t start, finish;
				start = clock();
				//im_flann_index.knnSearch(output1, indices, dists, im_Kneighbour, flann::SearchParams(256));
				finish = clock();
		...
		LumYcYb.push_back(CRt.clone());
		LumYcYb.push_back(CBt.clone());
		merge(LumYcYb, im_ResultImg);
		cvtColor(im_ResultImg, im_ResultImg, cv::COLOR_YCrCb2BGR);
		im_ResultImg = im_ResultImg * 255.0;
...
void SuperOperation::ProcessImage(string ResultFile)
{
	...
	for (int n = 0; n < iteration+1; n++)
	{	
		if (n == 0)
		{
			LoadTrainImg(count, interal);//2 startno number testno
			GenerateTrainSet();
			cout << "I have generate the trainset" << endl;
			//KdTreeBuild();
			PatchtableBuild();
			cout << "I have build the table" << endl;
		}
		else if (n >= count * im_FramePersInterval && n <= (count + 1) * im_FramePersInterval)
		{
			int number = n - 1;
			LoadTestImg(number);
			cout << "I have load testImg" << endl;
			cout << number << endl;
			//KdTreeMethod();
			PatchTableMethod();
			char file_name[50];
			sprintf(file_name, "result\\finalresult%d.png", number);
			cout <<"I have done patchtable method" << endl;
