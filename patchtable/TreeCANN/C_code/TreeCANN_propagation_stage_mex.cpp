...
//    mexPrintf("converting line arrays\n");
...
//            mexPrintf("coh_opt patch %d\n", n);
            ...
            if (x_map >= bew) { x_map = bew-1; }
            else if (x_map < 0) { x_map = 0; }
            if (y_map >= beh) { y_map = beh-1; }
            else if (y_map < 0) { y_map = 0; }
            
//            mexPrintf("coh_opt patch %d, %d %d %d %d (b effective size: %d %d)\n", n, x, y, x_map, y_map, bew, beh);

			for (int j = -(b_win-a_win)/2; j <=(b_win-a_win)/2 ; j++){
				for (int i = -(b_win-a_win)/2; i <= (b_win-a_win)/2; i++){
//					mexPrintf("coh_opt patch %d, i, j=%d, %d\n", n, i, j);
...
//					mexPrintf("coh_opt patch %d, i, j=%d, %d, finding minimum\n", n, i, j);
