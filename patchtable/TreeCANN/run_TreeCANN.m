function [nnf_dist nnf_X nnf_Y runtime] = run_TreeCANN (A, B, patch_w ,A_grid, B_grid, num_of_train_patches, num_PCA_dims, eps, num_of_ann_matches, A_win, B_win, second_phase, precompute_b)
...
% precompute_b -- Modify algo so image B is precomputed.
%     Report in time 7 the total lookup time.
%
% Outputs:
% - - - - - - - -
...
if (~exist('patch_w','var') || isempty(patch_w));   patch_w = 8; end
if (~exist('A_grid','var') || isempty(A_grid));     A_grid = 2   ; end
if (~exist('B_grid','var') || isempty(B_grid));     B_grid = A_grid; end

% init  First phase parameters (Kd-tree)
if (~exist('num_of_train_patches','var') || isempty(num_of_train_patches)); num_of_train_patches = 100;  end
if (~exist('num_PCA_dims','var') || isempty(num_PCA_dims)) ;                     num_PCA_dims = 3 + patch_w/2; end
if (~exist('num_of_ann_matches','var') || isempty(num_of_ann_matches));       num_of_ann_matches = 4;           end
if (~exist('eps','var') || isempty(eps));   eps = 3; end

% init  Second phase parameters (Propagation)
if (~exist('second_phase','var') || isempty(second_phase));  second_phase = 1; end

if (~exist('A_win','var') || isempty(A_win));    A_win = 2*A_grid+1;
elseif mod(A_win,2) == 0
        error('A_win parameter must be odd');
end

if (~exist('B_win','var') || isempty(B_win));   B_win = A_win+2;
elseif mod(B_win,2) == 0
        error('B_win parameter must be odd');
end

if (~exist('precompute_b','var') || isempty(precompute_b))
    precompute_b = 0;
end
...
if precompute_b
    tic;
    [A_patch_copy A_patch_copy2] = TreeCANN_reduce_patches(A, A, patch_w, A_grid, A_grid, PC_reduction);
    T_lookup = T_lookup + toc/2;
end

%fprintf('constructing kd-tree\n');
